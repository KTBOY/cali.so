...
// 本组件对外仅暴露中性的 FlashPlayer，页面与 UI 文案均不出现引擎名称。
declare global {
  interface Window {
    RufflePlayer?: {
      config?: Record<string, unknown>
      newest?: () => {
        createPlayer: (container: HTMLElement) => FlashPlayerElement
      }
    }
  }
}

interface FlashPlayerInstance {
...
type FlashPlayerElement = FlashPlayerInstance & HTMLElement
...
const FLASH_ENGINE_SRC = 'https://cdn.jsdelivr.net/npm/@ruffle-rs/ruffle/ruffle.js'

// 模块级单例：确保引擎脚本只加载一次
let engineScriptPromise: Promise<void> | null = null

// 抑制引擎 polyfill 扫描跨域 iframe 产生的无害警告
...
function loadEngineScript(): Promise<void> {
  ...
  engineScriptPromise = new Promise<void>((resolve, reject) => {
    if (window.RufflePlayer?.newest) {
      resolve()
      return
    }

    const existingScript = document.querySelector(`script[src="${FLASH_ENGINE_SRC}"]`)
    if (existingScript) {
      existingScript.addEventListener('load', () => resolve())
      existingScript.addEventListener('error', () =>
        reject(new Error('Failed to load flash engine'))
      )
      return
    }
    ...
    script.src = FLASH_ENGINE_SRC
    script.async = true
    script.onload = () => resolve()
    script.onerror = () => reject(new Error('Failed to load flash engine script'))
...
export default function FlashPlayer({
  ...
  const playerRef = useRef<FlashPlayerElement | null>(null)
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        ...
        // 创建播放器
        const engine = window.RufflePlayer.newest()
        const player = engine.createPlayer(container)
        playerRef.current = player

        // 此版本引擎 createPlayer 不会自动 append，需手动添加到容器
