...
- [components/GameUi/GameItem/gameItem.module.css](file://components/GameUi/GameItem/gameItem.module.css)
- [components/GameUi/GameItem/backBtn.css](file://components/GameUi/GameItem/backBtn.css)
- [components/GameUi/Page/Pagecomponent.tsx](file://components/GameUi/Page/Pagecomponent.tsx)
- [components/GameUi/Page/Pagecomponent.css](file://components/GameUi/Page/Pagecomponent.css)
...
- [app/(main)/game/page.tsx](file://app/(main)/game/page.tsx)
...
## 更新摘要
**变更内容**   
- 新增 GameUi 子模块文档，包含 RufflePlayer 游戏播放器、Card 卡片组件、GameItem 游戏项组件等
- 更新项目结构图以反映新的游戏中心 UI 架构
- 扩展领域组件章节，详细说明游戏相关组件的设计与使用
- 添加游戏中心页面集成示例
...
- components/GameUi：游戏相关领域组件（RufflePlayer 播放器、游戏卡片、游戏项、页面容器）
- components/links：链接类组件（富链接、社交链接）
- components/portable-text：Sanity Portable Text 渲染器（块、代码块、图片、推文）
- app/(main)：页面级业务组件与页面布局（博客、留言板、项目展示、导航、主题切换、游戏中心等）
...
game_center --> ruffle_player
game_center --> game_item
game_center --> game_page
ruffle_player --> swf_games
```
...
Tooltip <.. RichLink : "组合"
RufflePlayer <.. GameCenter : "组合"
GameCard <.. GameItem : "组合"
GameItem <.. GameCenter : "组合"
```
...
### 领域组件：GameUi 游戏中心模块
GameUi 子模块专为游戏中心功能设计，提供完整的 SWF 游戏播放解决方案。

#### RufflePlayer 游戏播放器
- 职责：基于 Ruffle 引擎的 SWF 游戏播放器，支持 Flash 内容在现代浏览器中运行。
- 属性约定：src（SWF 文件路径）、width、height、autoplay（自动播放）、loop（循环播放）、onLoad（加载完成回调）、onError（错误处理回调）。
- 特性：动态加载 Ruffle 引擎、内存管理、错误重试机制。
- 组合：被 GameCenter 和 GameItem 组件使用，提供游戏播放能力。

#### GameUi/Card 游戏卡片
- 职责：游戏列表中的卡片展示组件，包含游戏封面、标题、描述与播放按钮。
- 属性约定：title（游戏标题）、image（封面图片）、description（游戏描述）、playUrl（游戏地址）、onClick（点击播放回调）、className（样式覆盖）。
- 样式：独立的 Card.css 文件，支持响应式布局与主题适配。
- 组合：与 Button、Avatar 组合，形成完整的游戏信息展示。

#### GameUi/GameItem 游戏项组件
- 职责：单个游戏的完整封装，包含游戏数据管理、播放控制与导航功能。
- 数据流：从 data.ts 获取静态游戏数据，通过 utils.ts 提供数据处理工具函数。
- 样式：使用 gameItem.module.css 进行模块化样式管理，backBtn.css 提供返回按钮样式。
- 交互：支持游戏选择、收藏、分享等操作。

#### GameUi/Page 游戏页面容器
- 职责：游戏页面的统一布局容器，提供背景、导航与内容区域。
- 样式：Pagecomponent.css 定义页面整体样式与动画效果。
- 组合：与 Container 组合，确保响应式布局与内容居中。

#### swfGames 游戏数据管理
- 职责：集中管理所有 SWF 游戏的数据配置，包括游戏元信息、文件路径与分类标签。
- 数据结构：标准化的游戏对象数组，支持搜索、筛选与排序。
- 扩展：易于添加新游戏，只需在数据文件中注册即可。

```mermaid
graph LR
GameCenter["GameCenter"] --> RufflePlayer["RufflePlayer"]
GameCenter --> GameItem["GameItem"]
GameCenter --> GamePage["GamePage"]
GameItem --> GameCard["GameCard"]
GameItem --> BackBtn["BackBtn"]
RufflePlayer --> SWFGames["swfGames"]
GameItem --> Data["data.ts"]
GameItem --> Utils["utils.ts"]
GameCard --> CardCSS["Card.css"]
GameItem --> ItemCSS["gameItem.module.css"]
GamePage --> PageCSS["Pagecomponent.css"]
```
...
- 组件耦合度
  ...
  - GameUi 模块内部组件间存在适度耦合，但通过清晰的接口定义保持可维护性。
- 外部依赖
  - Tailwind 样式框架、Next.js 路由与 API 路由、Sanity 内容模型、Ruffle Flash 播放器引擎。
- 潜在循环依赖
  - 避免业务组件反向导入基础组件之外的其他业务组件，防止环状依赖。
  - GameUi 模块独立于其他业务模块，降低交叉依赖风险。
- 接口契约
  - 基础组件对外暴露稳定 props 与事件，业务组件不应修改其内部实现细节。
  - GameUi 组件提供稳定的游戏播放接口，便于后续扩展新的游戏类型。

```mermaid
graph TB
Base["基础组件(ui/*)"] --> Business["业务组件(app/(main)/*)"]
GameUi["GameUi 模块"] --> Base
Business --> External["外部依赖(API/第三方)"]
GameUi --> External
Base --> External
GameCenter["GameCenter"] --> GameUi
```
...
- 渲染优化
  ...
  - GameUi 模块中使用虚拟滚动优化大量游戏列表的渲染性能。
- 资源加载
  - 图片懒加载与占位，视频与第三方脚本按需加载（ClientOnly）。
  - Ruffle 引擎动态加载，仅在需要时初始化，减少初始包体积。
  - SWF 文件按需加载，避免一次性加载所有游戏资源。
- 交互优化
  - 防抖/节流处理高频事件（搜索、滚动）。
  - 大列表虚拟滚动（如 react-window）提升长列表性能。
  - 游戏播放器的内存管理，及时释放不再使用的 SWF 实例。
- 网络优化
  - 请求去重与缓存（React Query 或 SWR），失败重试与超时控制。
  - 游戏资源的 CDN 加速与预加载策略。
- 可访问性与 SEO
  - 语义化标签与 ARIA 属性，结构化数据与 meta 优化。
  - 游戏页面的可访问性支持，包括键盘导航与屏幕阅读器兼容。
...
- 常见问题
  ...
  - 游戏无法播放：检查 Ruffle 引擎加载状态、SWF 文件路径与 CORS 配置。
  - 游戏列表加载缓慢：检查 swfGames 数据量与图片懒加载配置。
- 调试建议
  - 使用浏览器开发者工具检查组件树与 props。
  - 在关键事件处添加日志，观察状态变化。
  - 针对网络请求，查看 API 路由返回码与错误信息。
  - 对于游戏相关问题，检查控制台中的 Ruffle 错误日志。
...
## 结论
本组件系统以基础组件为核心，通过组合与扩展满足多样化业务需求。新增的 GameUi 模块为游戏中心功能提供了完整的 UI 解决方案，包括游戏播放器、卡片展示、数据管理等核心能力。遵循单一职责、受控模式与可访问性原则，确保组件的可维护性与可扩展性。建议在后续迭代中完善测试覆盖率、文档站点与自动化发布流程，进一步提升团队协作效率与产品质量。
...
- GameUi 模块开发规范
  - 游戏数据统一管理在 swfGames.ts 中，保持数据结构一致性。
  - 游戏资源文件放置在 public/images/games/ 目录下，按游戏名称分类。
  - 游戏组件使用模块化 CSS 管理样式，避免全局样式污染。
...
- 单元与交互测试
  ...
  - GameUi 组件需要测试 Ruffle 引擎加载、SWF 文件播放等核心功能。
- 快照测试
  - 对稳定 UI 输出进行快照比对，注意合理拆分以避免过度敏感。
  - 游戏卡片的快照测试需要考虑不同分辨率下的布局表现。
- 端到端测试
  - 使用 Playwright/Cypress 验证跨组件工作流（如留言提交、文章阅读）。
  - 游戏中心的端到端测试包括游戏选择、播放、退出等完整流程。
...
- 示例路径
  ...
  - 游戏中心示例：[app/(main)/game-center/GameCenter.tsx](file://app/(main)/game-center/GameCenter.tsx)
...
- 版本管理
  ...
  - GameUi 模块的版本应与整体项目版本同步管理。
- 发布流程
  - 构建与测试通过后，打包并发布到私有仓库或 CDN。
  - 在文档站点同步更新组件版本与迁移指南。
  - 新增游戏时需要更新 swfGames 配置文件并进行兼容性测试。
- 回滚策略
  - 保留历史版本包，支持快速回滚与热修复。
  - 游戏资源的版本管理，支持回滚到特定游戏版本。
