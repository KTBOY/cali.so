...
B --> H["游戏中心模块<br/>game-center/*"]
B --> I["工具系统模块<br/>tools/*"]
H --> J["游戏中心页面<br/>GameCenter.tsx"]
I --> K["SWF转EXE工具<br/>swf-to-exe/*"]
I --> L["世界杯历史工具<br/>world-cup-history/*"]
M["React Query 提供者<br/>app/QueryProvider.tsx"] -.-> B
N["样式与主题配置<br/>globals.css / tailwind.config.cjs"] -.-> B
```

**更新** 新增了游戏中心和工具系统两个主要功能模块的路由结构
...
- **新增** 游戏与工具系统
  - GameCenter 组件提供游戏列表展示与选择功能
  - 工具系统采用模块化设计，每个工具独立成子路由
  - SWF 游戏播放器集成 Ruffle 技术实现 Flash 内容兼容
...
- 功能模块路由
  - game-center 提供游戏中心功能，包含游戏列表与选择界面
  - tools 提供工具系统，采用子路由结构组织不同工具
  - swf-to-exe 专门处理 SWF 文件转换为 EXE 格式
  - world-cup-history 展示世界杯历史信息
- 导航
...
Route --> |game-center| GameCenterPage["渲染游戏中心<br/>game-center/page.tsx"]
Route --> |tools| ToolsPage["渲染工具系统<br/>tools/page.tsx"]
ToolsPage --> SwfTool["SWF转换工具<br/>swf-to-exe/page.tsx"]
ToolsPage --> WorldCupTool["世界杯历史<br/>world-cup-history/page.tsx"]
Route --> |其他| DefaultPage["默认首页<br/>page.tsx"]
```

**更新** 新增了游戏中心和工具系统的完整路由结构，包括子工具页面的嵌套路由
...
### 游戏中心模块：交互式游戏体验
- 游戏列表展示
  - GameCenter 组件负责游戏网格布局与分类展示
  - 支持响应式设计，适配不同屏幕尺寸
- 游戏选择与加载
  - 点击游戏卡片进入游戏详情页
  - 集成 Ruffle 播放器实现 Flash 游戏兼容性
- 用户体验优化
  - 游戏封面图懒加载提升性能
  - 加载状态指示器改善用户体验
...
```mermaid
flowchart TD
Enter["进入游戏中心"] --> GameList["渲染游戏列表<br/>GameCenter.tsx"]
GameList --> Select["选择游戏"]
Select --> LoadGame["加载游戏页面"]
LoadGame --> Player["Ruffle 播放器"]
Player --> Play["开始游戏"]
Play --> Exit["退出游戏"]
Exit --> GameList
```

**新增** 游戏中心模块提供了完整的交互式游戏体验，采用模块化设计便于扩展新游戏
...
- 模块化架构
  ...
  - 统一的工具列表页面提供导航入口
- SWF 转换工具
  - SwfToExe 组件处理文件格式转换逻辑
  - 支持拖拽上传与进度反馈
- 世界杯历史工具
  - WorldCupHistory 组件展示历届世界杯数据
  - 集成图表可视化历史趋势
...
ToolsPage --> SwfToExe : "路由导航"
ToolsPage --> WorldCupHistory : "路由导航"
SwfToExe ..> WorldCupHistory : "独立组件"
```

**新增** 工具系统采用模块化设计，每个工具都是独立的组件和路由，便于后续添加新功能
...
- 关键耦合点
  ...
  - **新增** 游戏与工具：GameCenter/Ruffle -> 游戏资源
  - **新增** 工具系统：SwfToExe/WorldCupHistory -> 数据处理逻辑
...
Email["邮件模板"] --> API
GameCenter["游戏中心"] --> Games["游戏资源"]
Tools["工具系统"] --> Utils["工具逻辑"]
```

**更新** 新增了游戏中心和工具系统的依赖关系，展示了新的功能模块如何集成到整体架构中
...
- 调试建议
  ...
  - 对关键路径添加埋点与指标收集
- **新增** 游戏与工具问题排查
  - 游戏加载失败：检查 Ruffle 播放器资源与 Flash 文件完整性
  - 工具转换错误：验证输入文件格式与权限设置
  - 路由跳转异常：确认子路由配置文件与导航链接正确性
...
## 结论
本项目基于 Next.js App Router 实现了清晰的服务端渲染与客户端交互分层，结合 Tailwind 的主题与响应式能力，提供了良好的用户体验与可维护性。通过 React Query 与 API 路由解耦前后端数据流，配合 Portable Text 的富文本渲染与通用 UI 组件，形成了可扩展的前端架构。**新增的游戏中心与工具系统模块进一步丰富了网站功能，采用了模块化的设计模式，便于后续扩展与维护**。建议在后续迭代中继续强化错误边界、监控与测试覆盖率，进一步提升稳定性与开发效率。
